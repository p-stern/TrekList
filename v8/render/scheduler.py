import schedule
import time
import requests
import os
from models import db, connect_db, Media, Title
from flask import Flask

app = Flask(__name__)

# Get DB_URI from environ variable (useful for production/testing) or,
# if not set there, use development local db.
app.config['SQLALCHEMY_DATABASE_URI'] = (
    os.environ.get('DATABASE_URL', 'postgresql:///startrek'))

app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
app.config['SQLALCHEMY_ECHO'] = False
app.config['DEBUG_TB_INTERCEPT_REDIRECTS'] = False
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', "it's a secret")

connect_db(app)


def populate_db():

    drop_media_constraint = 'ALTER TABLE "title" DROP CONSTRAINT "fk_title_media_id"'
    # drop_title_constraint = 'ALTER TABLE "post" DROP CONSTRAINT "fk_post_title_id"'

    db.session.execute(drop_media_constraint)
    # db.session.execute(drop_title_constraint)
    db.session.commit()

    Title.query.filter(Title.media_id != 1).delete()
    db.session.execute("SELECT setval('title_id_seq', 14, true)")
    db.session.commit()
    print("Title table deleted")

    misnamed_episodes = ["Encounter at Farpoint (2)", "Emissary, Part II", "All Good Things... (2)", "Caretaker, Part II", "The Way of the Warrior, Part II",  "Dark Frontier, Part II",
                         "What You Leave Behind, Part II", "Flesh and Blood, Part II", "Endgame, Part II", "Broken Bow (2)"]
                        #  "Vitus Reflux", "Vox in Excelsio", "Series Acclimation Mil", "Come, Let's Away", "Episode 7", "Episode 8", "Episode 9", "Episode 10"]

    media = Media.query.order_by(Media.ord).all()
    for m in media:                  # Retrieve episodes for each TV Series and insert into 'title' table
        if m.media_type == 'TV':     # Do not include Movies
            url = "http://api.tvmaze.com/shows/" + str(m.id) + "/episodes"
            res = requests.get(url)
            data = res.json()

                         
            for d in data:
                if d['name'] not in misnamed_episodes:
                    # print(d['season'], d['name'], d['number'], d['airdate'])
                    raw_name = d['name']
                    if m.abbr == "Movie":
                        raw_url = "https://memory-alpha.fandom.com/wiki/" + d['name']
                    else:
                        raw_url = "https://memory-alpha.fandom.com/wiki/" + d['name'].replace('?', '%3F') + "_(episode)"
                
                
                    season, name, episode, airdate = corrections(
                        m.abbr, d['season'], d['name'], d['number'], d['airdate'])
                    

                    if m.abbr == "Movie":
                        validated_url = "https://memory-alpha.fandom.com/wiki/" + name
                    else:
                        validated_url = "https://memory-alpha.fandom.com/wiki/" + name.replace('?', '%3F') + "_(episode)"


                    validate(m.abbr, raw_url, raw_name, validated_url) 

                    # Calculate numeric value to be used for sorting
                    episode = str(episode)
                    episode_sort = int(episode.split('/')[0]) if episode.split('/')[0].isdigit() else None
                   
                    add_title(m.abbr, airdate, m.id, season,
                              episode, episode_sort, name, d['summary'])

    addSpecialEpisode(m)

    db.session.commit()

    add_media_constraint = 'ALTER TABLE "title" ADD CONSTRAINT "fk_title_media_id" FOREIGN KEY("media_id") REFERENCES "media" ("id")'
    # add_title_constraint = 'ALTER TABLE "post" ADD CONSTRAINT "fk_post_title_id" FOREIGN KEY("title_id") REFERENCES "title" ("id");'

    db.session.execute(add_media_constraint)
    # db.session.execute(add_title_constraint)
    db.session.commit()

    print("Loaded")
    print("Cntl-Z to exit")
    return schedule.CancelJob

def validate(abbr, raw_url, raw_name, validated_url):
    raw_res = requests.head(raw_url)
    raw_valid = True
    if raw_res.status_code == 404:
        raw_valid = False
            
    validated_res = requests.head(validated_url)
    validated_valid = True
    if validated_res.status_code == 404:
        validated_valid = False

    if raw_valid and validated_valid:
        # Does not need to be in corrections() 
        pass    

    if raw_valid and not validated_valid:
        # Changed in corrections() incorrectly
        print(f"Remove episode from corrections(): [{abbr}] {raw_name} ")
    
    if not raw_valid and validated_valid:
        # Changed in corrections() correctly
        pass
    
    if not raw_valid and not validated_valid:
        # Needs to be in corrections() correctly
        print(f"Invalid URL.  Add or update episode in corrections(): [{abbr}] {raw_name} ")


def add_title(abbr, airdate, id, season, episode, episode_sort, name, summary):
    title = Title(abbr=abbr, premiered_date=airdate, media_id=id,
                  season_id=season, episode_id=episode, episode_num_sort=episode_sort, title=name, summary=summary)
    db.session.add(title)


def addSpecialEpisode(m):
    """Get Pilot episode (which has episode None)"""

    url = "http://api.tvmaze.com/seasons/1921/episodes"

    res = requests.get(url)
    data = res.json()
    for d in data:
        if d.get('number') is None:
            season, name, episode, airdate = corrections(
                m.abbr, d['season'], d['name'], d['number'], d['airdate'])
            episode = str(episode)
            episode_sort = int(episode.split('/')[0]) if episode.split('/')[0].isdigit() else None

            add_title('TOS', airdate, 490, season, episode, episode_sort, name, d['summary'])

def corrections(abbr, season, name, episode, airdate):

    if airdate == "":
        airdate = '9999-12-31'

    # """Tweak titles so that the a href points to an existing url"""

    # TOS

    if name == "What Are Little Girls Made of?":
        name = "What Are Little Girls Made Of?"

    if name == "The Menagerie Part I":
        name = "The Menagerie, Part I"

    if name == "The Menagerie Part II":        
        name = "The Menagerie, Part II"

    if name == "Tomorrow Is Yesterday":
        name = "Tomorrow is Yesterday"

    if name == "Operation: Annihilate!":
        name = "Operation -- Annihilate!"

    if name == "The Cage":
        season = 0
        episode = 0
        airdate = '1988-10-04'

    # TNG   

    if name == "Encounter at Farpoint (1)":
        name = "Encounter at Farpoint"
        episode = "1/2"

    if name == "Hide and Q":
        name = "Hide And Q"
    
    if name == "Too Short a Season":
        name = "Too Short A Season"

    if name == "When the Bough Breaks":
        name = "When The Bough Breaks"

    if name == "Skin of Evil":
        name = "Skin Of Evil"

    if name == "Loud as a Whisper":
        name = "Loud As A Whisper"

    if name == "A Matter of Honor":
        name = "A Matter Of Honor"

    if name == "The Measure of a Man":
        name = "The Measure Of A Man"

    if name == "Q Who?":
        name = "Q Who"

    if name == "Up the Long Ladder":
        name = "Up The Long Ladder"

    if name == "Who Watches the Watchers":
        name = "Who Watches The Watchers"

    if name == "Déjà Q":
        name = "Deja Q"

    if name == "Sins of the Father":
        name = "Sins of The Father"

    if name == "The Best of Both Worlds Part II":
        name = "The Best of Both Worlds, Part II"

    if name == "A Matter of Time":
        name = "A Matter Of Time"

    if name == "Cause and Effect":
        name = "Cause And Effect"

    if name == "Cost of Living":
        name = "Cost Of Living"
    
    if name == "I, Borg":
        name = "I Borg"

    if name == "Realm of Fear":
        name = "Realm Of Fear"

    if name == "Man of the People":
        name = "Man Of The People"

    if name == "Chain of Command, Part I":
        name = "Chain Of Command, Part I"

    if name == "Chain of Command, Part II":
        name = "Chain Of Command, Part II"

    if name == "Ship in a Bottle":
        name = "Ship In A Bottle"

    if name == "Face of the Enemy":
        name = "Face Of The Enemy"

    if name == "Descent, Part I":
        name = "Descent"
    
    if name == "All Good Things... (1)":
        name = "All Good Things..."
        episode = "25/26"

    # DS9

    if name == "Emissary, Part I":
        name = "Emissary"
        episode = "1/2"

    if name == "The Way of the Warrior, Part I":
        name = "The Way of the Warrior"
        episode = "1/2"      

    if name == "...Nor the Battle to the Strong":
        name = "Nor the Battle to the Strong"

    if name == "Let He Who is Without Sin...":
        name = "Let He Who Is Without Sin..."
    
    if name == "What You Leave Behind, Part I":
        name = "What You Leave Behind"
        episode = "25/26"

    # VOY

    if name == "Caretaker, Part I":
        name = "Caretaker"
        episode = "1/2"

    if name == "Future's End, Part I":
        name = "Future's End"

    if name == "The Darkling":
        name = "Darkling"

    if name == "Scorpion, Part I":
        name = "Scorpion"

    if name == "Year of Hell, Part I":
        name = "Year of Hell"

    if name == "The Killing Game, Part I":
        name = "The Killing Game"

    if name == "Dark Frontier, Part I":
        name = "Dark Frontier"
        episode = "15/16"

    if name == "Equinox, Part I":
        name = "Equinox"

    if name == "Tinker, Tenor, Doctor, Spy":
        name = "Tinker Tenor Doctor Spy"

    if name == "Unimatrix Zero, Part I":
        name = "Unimatrix Zero"

    if name == "Flesh and Blood, Part I":
        name = "Flesh and Blood"
        episode = "9/10"

    if name == "Workforce, Part I":
        name = "Workforce"

    if name == "Endgame, Part I":
        name = "Endgame"
        episode = "25/26"

    # ENT

    if name == "Broken Bow (1)":
        name = "Broken Bow"
        episode = "1/2"

    if name == "Shockwave (1)":
        name = "Shockwave"

    if name == "Shockwave (2)":
        name = "Shockwave, Part II"

    if name == "A Night in Sickbay":
        name = "A Night In Sickbay"

    if abbr == "ENT" and name == "Anomaly":
        name = "Anomaly (ENT)"

    if name == "Storm Front (1)":
        name = "Storm Front"

    if name == "Storm Front (2)":
        name = "Storm Front, Part II"

    if name == "Borderland (1)":
        name = "Borderland"      

    if name == "Cold Station 12 (2)":
        name = "Cold Station 12"

    if name == "The Augments (3)":
        name = "The Augments"

    if name == "The Forge (1)":
        name = "The Forge"

    if name == "Awakening (2)":
        name = "Awakening"

    if name == "Kir'Shara (3)":
        name = "Kir'Shara"

    if name == "Babel One (1)":
        name = "Babel One"

    if name == "United (2)":
        name = "United"

    if name == "The Aenar (3)":
        name = "The Aenar"

    if name == "Affliction (1)":
        name = "Affliction"

    if name == "Divergence (2)":
        name = "Divergence"

    if name == "In a Mirror, Darkly (1)":
        name = "In A Mirror, Darkly"

    if name == "In a Mirror, Darkly (2)":
        name = "In A Mirror, Darkly, Part II"

    if name == "Demons (1)":
        name = "Demons"

    if name == "Terra Prime (2)":
        name = "Terra Prime"

    if name == "These Are the Voyages...":
        name = "These Are The Voyages..."

    # DIS

    if name == "The War Without, the War Within":
        name = "The War Without, The War Within"

    if name == "The Sounds of Thunder":
        name = "The Sound of Thunder"
    
    if name == "Such Sweet Sorrow, Part 1":
        name = "Such Sweet Sorrow"
    
    if name == "There Is a Tide…":
        name = "There Is A Tide..."

    if abbr == "DIS" and name == "Anomaly":
        name = "Anomaly (DIS)"

    if name == "…But to Connect":
        name = "...But to Connect"

    # ST

    if name == "Ephraim & Dot":
        name = "Ephraim and Dot"

    # PIC

    if name == "The End Is the Beginning":
        name = "The End is the Beginning"

    if name == 'Part One: "The Next Generation"':      
        name = "The Next Generation"
        
    if name == 'Part Two: "Disengage"':
        name = "Disengage"

    if name == 'Part Three: "Seventeen Seconds"':
        name = "Seventeen Seconds"

    if name == 'Part Four: "No Win Scenario"':
        name = "No Win Scenario"
        
    if name == 'Part Five: "Imposters"':
        name = "Imposters"
        
    if name == 'Part Six: "The Bounty"':
        name = "The Bounty"
        
    if name == 'Part Seven: "Dominion"':
        name = "Dominion"
        
    if name == 'Part Eight: "Surrender"':
        name = "Surrender"
        
    if name == 'Part Nine: "Võx"':
        name = "Võx"
        
    if name == 'Part Ten: "The Last Generation"':
        name = "The Last Generation"

    # LD

    if name == "An Embarrassment of Dooplers":
        name = "An Embarrassment Of Dooplers"

    if name == "Mining the Mind's Mines":
        name = "Mining The Mind's Mines"

    if name == "The Stars at Night":
       name = "The Stars At Night"

    if name == "A Farewell to Farms":
        name = "A Farewell To Farms"

    # SNW

    if name == "Ghost of Illyria":
        name = "Ghosts of Illyria"
        
    if name == "Ad Astra Per Aspera":
        name = "Ad Astra per Aspera"

    if name == "Four-And-A-Half-Vulcans":
        name = "Four-and-a-Half Vulcans"

    #PRO

    if abbr == "PRO" and season == 1:
        if name == "Lost and Found":
            episode = "1/2"
        else:
            episode = episode + 1

    # SA

    if name == "Vox in Excelso":
        name = "Vox In Excelso"

    return season, name, episode, airdate

schedule.every(10).seconds.do(populate_db)

while 1:
    schedule.run_pending()
    time.sleep(1)